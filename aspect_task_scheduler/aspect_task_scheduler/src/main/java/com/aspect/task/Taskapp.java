...
public class Taskapp {
	...
	public Response getAverageWaitTime() {
		...
		output = output+" Task has average wait time of :"+(current_seconds-(average_time/arr.length));
		return Response.status(200).entity(output).build();
	...
	@DELETE
	@Path("/gettopid")
	public Response getTopID() {
		
		return Response.status(200).entity(Task_Utility.getTopID()).build();
